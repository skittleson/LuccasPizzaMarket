...
function saveHandlebarsToHtml(inFile, outFile, data) {
  ...
  if (isMarkdown) {
    data.contents = markdown.render(source);
    data.contents = handlebars.compile(data.contents, { strict: true })(data);
  } else {
    data.contents = handlebars.compile(source, { strict: true })(data);
  }
  let templateHbs = "src/partials/standard.hbs";
  if (data.template) {
    templateHbs = "src/partials/" + data.template;
  }
...
module.exports.buildSiteFromJson = function buildSiteFromJson(json, src, dest) {
  ...
  store.pages.forEach(page => {
    let pageKeyValue = {};
    Object.assign(pageKeyValue, store.site);
    Object.assign(pageKeyValue, page);
  ...
  // Create manifest feed
  let manifest = fs.readFileSync(`${src}/manifest.hbs`, "utf8");
  fs.writeFileSync(
    `${dest}/manifest.json`,
    handlebars.compile(manifest, { strict: true })({
      site: store.site,
      posts: posts
    }),
    {}
